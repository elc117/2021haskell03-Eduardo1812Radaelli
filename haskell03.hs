...
add10toall :: [Int] -> [Int]
add10toall lstint = [x + 10 | x <- lstint] 
...
addSuffix sufixo lststr = [x ++ sufixo | x <- lststr] 
...
selectgt5 :: [Int] -> [Int]
selectgt5 lstint = [x | x <- lstint, x > 5]
...
selectExpr :: [Int] -> [Int]
selectExpr lstint = [x | x <- lstint, x `mod` 2 == 0, x > 20, x < 50]
...
countShorts lststr = length [x | x <- lststr, length x < 5]
...
calcExpr lstfloat = [x^2/2 | x <- lstfloat, (x^2/2) > 10]
...
selectSnd lsttuplas = [snd x | x <- lsttuplas]
...
dotProd :: [Int] -> [Int] -> Int
dotProd lstint lstint2 = sum [fst x * snd x | x <- zip lstint lstint2]